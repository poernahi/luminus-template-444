...
The `home-routes` handler in the `luminus-template-444.routes.home` namespace
...
The `home-page` function will in turn call the `luminus-template-444.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `luminus-template-444.handler` namespace:
...
Request middleware functions are located under the `luminus-template-444.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `luminus-template-444.dev-middleware` namespace found in
